...
**No measurement ID, no banner.** The banner asks permission for exactly one
thing, so while `GA_MEASUREMENT_ID` is empty — or holds something that is not a
measurement ID — no banner is shown at all rather than an empty question
nothing can act on. Paste an ID in and it appears on every page again; nothing
else changes.
